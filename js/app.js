...
jQuery(function() {
	...
	if(element_exists('.animateSVG')) {
		...
		var tween = new TimelineMax()
			.add(TweenMax.to($word, 0.1, {strokeDashoffset: 0.75, ease:Power0.easeNone}))
		var scene = new ScrollMagic.Scene({triggerElement: "#content", duration: 550, tweenChanges: true})
			.setTween(tween)
			.addTo(controller);
	}
...
function initMap() {
	var map = new google.maps.Map(document.getElementById('map'), {
		...
		styles: [
			...
			{elementType: 'labels.text.fill', stylers: [{color: '#4f535e'}]},
			{elementType: 'labels.text.stroke', stylers: [{color: '#e8b84e'}]},
			{
